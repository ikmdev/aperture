...
@Component
public class DefinitionProcessor implements KnowledgeProcessor{
	...
	public void process(SolorGenerationContext solorGenerationContext, PublicId conceptId) {
		...
		solorRequest.stampCalculatorWithCache().forEachSemanticVersionForComponentOfPattern(
				...
				(semanticEntityVersion, _, _) -> {
					if (semanticEntityVersion.fieldAsConceptFacade(3).nid() == TinkarTermV2.DEFINITION_DESCRIPTION_TYPE.nid()) {
						// Get Definition from Semantic
						String definition = semanticEntityVersion.fieldAsString(1);
						...
						Resource descriptionNode = model.createResource()
								.addProperty(SolorVocabulary.HAS_DEFINITION, definition, solorRequest.languageCoordinate().getISOCode())
						...
						// Add Definition to Concept Resource in the model
